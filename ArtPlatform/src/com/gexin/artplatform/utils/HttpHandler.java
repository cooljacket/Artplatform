...
public class HttpHandler extends Handler {
	...
	public void handleMessage(Message message) {
		...
		case HttpConnectionUtils.DID_SUCCEED: // connection success
			...
			DLog.i(context.getClass().getSimpleName(),
					"http connection return." + response);
			try {
				JSONObject jObject = new JSONObject(response == null ? ""
						: response.trim());
				succeed(jObject);
				// if ("true".equals(jObject.getString("success"))) { //operate
				// success
				// Toast.makeText(context,
				// "operate succeed:"+jObject.getString("msg"),Toast.LENGTH_SHORT).show();
				// succeed(jObject);
				// } else {
				// Toast.makeText(context,
				// "operate failed:"+jObject.getString("msg"),Toast.LENGTH_LONG).show();
				// failed(jObject);
				// }
				...
				e1.printStackTrace();
				Toast.makeText(context, "数据解析出错", Toast.LENGTH_LONG).show();
			}
			break;
		case HttpConnectionUtils.DID_ERROR: // connection error
			...
			e.printStackTrace();
			DLog.e(context.getClass().getSimpleName(),
					"connection fail." + e.getMessage());
			Toast.makeText(context,
					context.getResources().getString(R.string.connection_fail),
